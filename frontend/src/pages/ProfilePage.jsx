...
// ─── mock data — replace with API fetch by userNo ────────────────────────────
const MOCK_PROFILES = {
  1: {
    userNo: 1,
    name: "Arjun Sharma",
...
export default function ProfilePage() {
  ...
  const isOwner = String(userNo) === String(currentUserNo);

  const [profile, setProfile]   = useState(MOCK_PROFILES[userNo] || null);
  ...
  const color  = avatarColor(profile.userNo);
          ...
          <div style={{ maxWidth: "900px", margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
            <div>
              <p style={{ fontSize: "11px", color: "#FF4D4D", letterSpacing: "3px", textTransform: "uppercase", fontFamily: "'Courier New', monospace", marginBottom: "0.3rem" }}>
                — Profile #{String(profile.userNo).padStart(3, "0")}
              </p>
